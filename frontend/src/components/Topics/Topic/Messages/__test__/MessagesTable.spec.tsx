...
describe('MessagesTable', () => {
  beforeEach(() => {
    ...
    jest.mocked(useTimezone).mockReturnValue({
      currentTimezone: timezone,
      availableTimezones: [timezone],
      setTimezone: jest.fn(),
      getDateInCurrentTimezone: jest.fn((date = new Date()) => date),
    });
  });
  ...
  describe('should render Messages table with data', () => {
    ...
    it('reads the shared timezone once per table render instead of once per message row', () => {
      jest.mocked(useTimezone).mockClear();
      renderComponent({
        messages: [0, 1, 2].map((index) => ({
          ...topicMessagePayload,
          offset: topicMessagePayload.offset + index,
        })),
        isFetching: false,
      });

      // Initial render plus the existing preview-filter hydration render.
      expect(useTimezone).toHaveBeenCalledTimes(2);
    });
  });
