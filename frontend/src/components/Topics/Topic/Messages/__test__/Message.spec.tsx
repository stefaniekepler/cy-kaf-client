...
jest.mock('lib/hooks/useDataSaver', () =>
  jest.fn(() => ({
...
const mockUseDataSaver = jest.mocked(useDataSaver);
...
describe('Message component', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockUseDataSaver.mockReturnValue({
      copyToClipboard: jest.fn(),
      saveFile: jest.fn(),
    });
  ...
  it('defers message action setup until the row is hovered', async () => {
    renderComponent();
    expect(mockUseDataSaver).not.toHaveBeenCalled();

    await act(() => userEvent.hover(screen.getByRole('row')));

    expect(mockUseDataSaver).toHaveBeenCalledTimes(1);
  });

  it('memoizes unchanged message rows', () => {
    expect(
      (Message as React.MemoExoticComponent<React.FC<Props>>).$$typeof
