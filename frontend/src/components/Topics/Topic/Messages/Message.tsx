...
const MessageActions: React.FC<{ message: TopicMessage }> = ({ message }) => {
  const { topicName } = useAppParams<RouteParamsClusterTopic>();
  const { openSidebarWithMessage } = useTopicActions();
  const { value, offset, key, partition, headers, timestamp } = message;
      ...
      >
        ...
        <td style={{ width: '5%' }}>
          <div style={{ visibility: vEllipsisOpen ? 'visible' : 'hidden' }}>
            {actionsMounted && <MessageActions message={message} />}
          </div>
        </td>
      </S.ClickableRow>
      {isOpen && (
