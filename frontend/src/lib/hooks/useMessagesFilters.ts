...
export function useMessagesFilters(topicName: string) {
  ...
  const [initializedResource, setInitializedResource] = useState<string>();
  const isInitialized = initializedResource === storageKey;
  ...
  useEffect(() => {
    const initializedParams = new URLSearchParams(searchParams);
    initMessagesFiltersFields(initializedParams);
    initializedParams.set(MessagesFilterKeys.limit, PER_PAGE.toString());

    if (initializedParams.get(MessagesFilterKeys.mode) === null) {
      initializedParams.set(MessagesFilterKeys.mode, defaultModeValue);
    }

    initializedParams.delete(MessagesFilterKeys.cursor);

    setInitializedResource(undefined);
    setInitializationTarget({
      resourceName: storageKey,
      search: initializedParams.toString(),
    });
    setSearchParams(initializedParams, { replace: true });
  }, [storageKey]);

  useEffect(() => {
    if (
      initializationTarget?.resourceName === storageKey &&
      searchParams.toString() === initializationTarget.search &&
      initializedResource !== storageKey
    ) {
      setInitializedResource(storageKey);
    }
  }, [initializationTarget, initializedResource, searchParams, storageKey]);
...
export type MessagesFilters = ReturnType<typeof useMessagesFilters>;
