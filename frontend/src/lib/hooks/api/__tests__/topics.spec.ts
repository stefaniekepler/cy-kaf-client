...
describe('Topics hooks', () => {
  ...
  describe('useTopicAnalysis', () => {
    ...
    it('suppresses only the expected analysis 404 from global reporting', () => {
      const { result } = renderHook(
        () => {
          hooks.useTopicAnalysis(topicParams, false);
          return useQueryClient();
        },
        { wrapper: TestQueryClientProvider }
      );
      const query = result.current.getQueryCache().find({
        queryKey: hooks.topicKeys.statistics(topicParams),
      });

      expect(query?.meta).toEqual({
        suppressGlobalErrorStatuses: [404],
      });
    });
  });
