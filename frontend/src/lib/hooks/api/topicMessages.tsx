...
export const useTopicMessages = ({
  ...
  const pendingMessages = useRef<TopicMessage[]>([]);
  const pendingFrame = useRef<number>();
  const activeMode = useRef<PollingMode>();
  ...
  const flushPendingMessages = useCallback(() => {
    ...
    setMessages((previous) =>
      activeMode.current === PollingMode.TAILING
        ? [...batch].reverse().concat(previous)
        : previous.concat(batch)
    );
  }, []);
  ...
  React.useEffect(() => {
    ...
    const modeParam = searchParams.get(MessagesFilterKeys.mode) ?? '';
    const mode = convertStrToPollingMode(modeParam);
