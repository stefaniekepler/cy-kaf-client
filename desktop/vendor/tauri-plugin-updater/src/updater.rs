...
const MAX_UPDATE_JSON_BYTES: u64 = 128 << 10;

/// Internal compatibility hook for cy-kaf-client's bounded-response tests.
#[doc(hidden)]
pub async fn read_update_json(response: reqwest::Response) -> Result<serde_json::Value> {
    if response
        .content_length()
        .is_some_and(|length| length == 0 || length > MAX_UPDATE_JSON_BYTES)
    {
        return Err(Error::Network("update metadata exceeds 128 KiB".into()));
    }
    ...
    while let Some(chunk) = stream.next().await {
        ...
        let length = (bytes.len() as u64)
            ...
            .ok_or_else(|| Error::Network("update metadata is too large".into()))?;
        if length > MAX_UPDATE_JSON_BYTES {
            return Err(Error::Network("update metadata exceeds 128 KiB".into()));
        }
        bytes.extend_from_slice(&chunk);
    }
    if bytes.is_empty() {
        return Err(Error::Network("update metadata is empty".into()));
    }
            ...
            match response {
                Ok(res) => {
                    if res.status().is_success() {
                        ...
                        let update_response = read_update_json(res).await?;
