...
| 2026-09-09 | `frontend/src/components/Dashboard/{ClusterTableActionsCell.tsx,Dashboard.tsx,Dashboard.styled.ts}`、`frontend/src/lib/duplicateClusterName.ts`、`frontend/src/lib/hooks/api/appConfig.ts` 及对应测试 | Clusters 列表的 Configure 之后新增 Duplicate：二次确认后按现有 `GET /api/config` → `PUT /api/config` 通路追加一份完整克隆，名称为 `<原名>-copy`，被占用时递增为 `-copy-2`、`-copy-3`…；actions 列宽 140→220 以容纳两个按钮 | 用户批准的环境配置克隆入口；复用既有配置写入链路与权限门控（`hasDynamicConfig` + `APPLICATIONCONFIG`），未新增后端端点或契约变更 |
| 2026-09-09 | `frontend/src/components/Settings/{ConfigTransferSection.tsx,SettingsModal.tsx,UpdateSection.tsx}` 及对应测试 | 删除环境配置区的 YAML 说明段落；MCP 区标题改为「MCP配置」；Updates 与 Diagnostics 合并为最下方的「日志与更新」单一区块（`UpdateSection` 不再自带 section 外壳，浏览器模式仍显示 desktop-only 提示） | 用户批准的 Settings 界面精简；仅调整分区与标题文案，更新检查、日志目录与 MCP 的行为、按钮文案均未改动 |
| 2026-09-09 | `frontend/src/components/Settings/{SettingsModal.tsx,SettingsModal.styled.ts,UpdateSection.tsx}` 及对应测试 | 将上一行合并出的「日志与更新」重新拆为两个区块：「日志」（保留日志目录动作与 desktop-only 提示）与其后的「更新」（`UpdateSection` 恢复自带 section 外壳，非桌面宿主时整段不渲染）；「You are up to date.」改用主题浅绿 `circularAlert.color.success`（#5CD685） | 用户反馈合并后信息过于杂乱，要求拆分并让"已是最新版本"提示更醒目；仅调整分区与该行文案颜色，更新检查/安装、日志目录与 MCP 行为均未改动 |
| 2026-09-09 | `frontend/src/components/Dashboard/{ClusterTableActionsCell.tsx,Dashboard.tsx,Dashboard.styled.ts}`、`frontend/src/lib/hooks/api/appConfig.ts` 及对应测试 | Clusters 列表操作列：Configure 改浅蓝底（`tag.backgroundColor.blue`），新增末位 Delete 浅红底（`tag.backgroundColor.red`）按钮，经 danger 二次确认后调用新增的 `useDeleteAppConfigCluster` 删除该环境配置；actions 列宽 220→290 | 用户要求区分操作按钮并提供带二次确认的环境删除入口；删除复用既有 `PUT /api/config` 写入链路与 `aggregateClusters` 删除语义，未新增后端端点 |
| 2026-09-09 | `frontend/src/components/Dashboard/{ClusterTableActionsCell.tsx,Dashboard.styled.ts}` 及对应测试 | 删除环境的二次确认弹窗补一行提醒：「该操作仅删除本地配置，不影响 Kafka 服务端。」 | 用户要求在确认环节说明删除范围，避免误以为会动 Kafka 集群本身；仅新增弹窗文案，删除行为不变 |
| 2026-09-09 | `frontend/src/components/Settings/{UpdateSection.tsx,SettingsModal.styled.ts}` 及对应测试 | 「You are up to date.」由 `#5CD685` 改为更暗的 `#29a352`（调色板 green[60]），并从标题下方移到「Check for updates」按钮右侧同一行、间隔 12px；显示条件不变（仍只在 `status === 'up_to_date'` 时出现） | 用户反馈原绿色过亮且位置突兀；核对 Rust 侧 `updates/worker.rs` 后确认 `up_to_date` 只由真实检查返回「无更新」时产生，无需改门控，仅补测试锁死该语义 |
| 2026-09-09 | `frontend/src/theme/theme.ts`、`frontend/src/components/Dashboard/Dashboard.tsx` | 侧边栏宽度 `layout.navBarWidth` 240px → 288px（+20%，深浅主题共用该 token）；Clusters 表列宽重排：Cluster name 显式 `size: 240`（列宽占比 14.2%→21.2%，约 +50%），Version 100→90、Brokers count 120→124、Partitions 100→100（表格按 size 比例分配总宽，侧边栏变宽后三列实际渲染由 114/136/114 收窄至 91/125/101） | 用户要求加宽侧边栏与集群名列、收窄中间三列；已在 1440/1280/1152 三档视口实测三列渲染宽度均大于「表头文本 + th 24px + 排序图标 14px」的最小需求，列名不换行且页面无横向溢出 |
| 2026-09-09 | `frontend/src/components/PageContainer/useAllClustersReminder.ts`、`frontend/src/components/Nav/{Nav.tsx,ClusterMenu/ClusterMenu.tsx,Menu/MenuItem.tsx}`、`frontend/src/lib/hooks/useLocalStorage.ts` 及对应测试 | 「返回全部集群」提醒改为每次打开 App 只出现一次（模块级会话标记，导出 `resetAllClustersReminderSession` 供测试复位），不再每进入一个新集群提醒；点击侧边栏「All clusters」时把所有集群菜单收起（新增导出的 `writeLocalStorageValue` 复用 `useLocalStorage` 既有订阅广播，`MenuItem` 增加可选 `onClick`，`ClusterMenu` 导出 `clusterMenuOpenKey`） | 用户反馈提醒过于频繁、且返回全部集群后侧边栏仍保持展开；折叠状态沿用既有 localStorage 持久化，刷新后保持收起 |
